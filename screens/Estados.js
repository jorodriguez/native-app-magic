...
import { getBalance } from '../servicios/BalanceService';
...
export default class Estados extends React.Component {
  ...
  getBalance = () => {
    ...
    getBalance(this.state.usuarioSesion.id, this.state.token)
      .then(res => res.json())
      .then(res => {
  ...
  handleResponse(response, handlerProcess) {
    if (!response.estatus) {
      ...
      if (response.tokenExpirado) {
        //Alert.alert("token", " token expirado " + response.tokenExpirado);
